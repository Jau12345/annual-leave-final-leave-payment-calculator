...
function calculateOutstandingLeave() {
    ...
    document.getElementById('carryForwardResult').textContent = usedCarryBeforeApr ? daysUsedBeforeApr : 0;
...
function calculateFinalLeavePayment() {
    ...
    rows.forEach(row => {
        ...
        if (type === "Salary") salaryTotal += payment;
        if (type === "Non-Compete") nonCompeteTotal += payment;
        paymentTableRows += `<tr>
            <td>${month} ${type}</td>
            <td>${workingDays}</td>
    ...
    document.getElementById('finalLeaveResult').innerHTML = `
        ...
        <p><strong>Total Salary:</strong> HKD ${salaryTotal.toLocaleString()}</p>
        <p><strong>Total Garden Leave:</strong> HKD ${gardenLeaveTotal.toLocaleString()}</p>
        <p><strong>Total Non-Compete:</strong> HKD ${nonCompeteTotal.toLocaleString()}</p>
        <table border="1" cellpadding="5" style="border-collapse:collapse;width:100%;">
